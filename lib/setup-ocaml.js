...
async function acquireOpamWindows(version, variant, customRepository) {
...
async function acquireOpamLinux(version, variant, customRepository) {
  ...
  await exec.exec("sudo apt-get -y install bubblewrap ocaml-native-compilers ocaml-compiler-libs musl-tools dos2unix git darcs mercurial rsync pkg-config m4");
  await exec.exec(`"${toolPath}/opam"`, ["init", "-yav", repository]);
  await exec.exec(path.join(__dirname, "install-ocaml-unix.sh"), [version, variant]);
  await exec.exec(`"${toolPath}/opam"`, ["install", "-y", "depext"]);
}
async function acquireOpamDarwin(version, variant, customRepository) {
  const repository = customRepository || "https://github.com/ocaml/opam-repository.git";
